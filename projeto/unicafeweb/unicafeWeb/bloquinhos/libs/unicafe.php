...
function unicafeConnect($servidor = null, $usuario = null, $senha = null) {
	...
	if (false == ($result = @socket_connect ( $socket, $host, $port ))) {
		throw new UniCafeException ( "Erro de conex�o!" );
		return false;
	}
	if (false == ($result = socket_write ( $socket, $message, strlen ( $message ) ))) {
		throw new UniCafeException ( "Conex�o recusada!" );
		return false;
	}
	return $socket;
}
...
/**
 * Envia Sql para o UniCafe para que possa retornar dados de m�quinas conectadas.
 *
 * @param resource $link        	
 * @param string $query        	
 */
...
/**
 * Fun��o que conversa com o servidor a baixo n�vel, enviando comandos e esperando resposta completa.
 *
 * @param resource $link        	
 * @param string $query        	
 */
...
/**
 * Fecha uma conex�o com o UniCafe.
 * 
 * @param resource $link        	
 */
